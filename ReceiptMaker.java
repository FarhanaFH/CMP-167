...
/*ReceiptMaker for making receipt
 */
public class ReceiptMaker {
	...
	public ReceiptMaker() {
	    ...
	    MAX_NUM_ITEMS = 10;  // Maximum number of items 
	    TAX_RATE = .0875;   // Tax rate 
	    itemNames = new String[MAX_NUM_ITEMS];  // Array to store item names
	    itemPrices = new double[MAX_NUM_ITEMS]; // Array to store item prices
	    numItemsPurchased = 0;  // Number of items purchased
	}

	public ReceiptMaker(int maxNumItems, double taxRate) {
	    // Initialize values based on the provided parameters
	    MAX_NUM_ITEMS = maxNumItems;  // Maximum number of items 
	    TAX_RATE = taxRate;   // Tax rate for the items
	    itemNames = new String[MAX_NUM_ITEMS];  // Array to store item names
	    itemPrices = new double[MAX_NUM_ITEMS]; // Array to store item prices
	    numItemsPurchased = 0;  // Number of items purchased
	}
    ...
    public static void main(String [] args) {
        ...
        // Display the receipt & statistics
        rm.displayReceipt();
        rm.displayReceiptStats();
        
        // Display all items & Close scanner
